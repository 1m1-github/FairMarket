...
A **market** $\mathcal{M}$ is a function that determines the *next* **bid** to be [serviced](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#all-trade-is-sequential). Formally,
...
In this paper, we will define a partial ordering and a total ordering that is **[fairest](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#fairest)**, most **[inclusive](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#most-inclusive)** and accomodates [all](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#all-markets) types of **market**s, as well as innovating new types of **market**s with a generic framework.
...
Let $\mathcal{C}$ be the universe of all existing [currencies](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#currency).

Then

$$\mathcal{C} = \mathcal{C}_\text{obj} \text{  } \dot{\cup} \text{  } \mathcal{C}_\text{sub}$$

that is, any **currency** either has [objective value xor subjective value](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#objective-vs-subjective-value).
...
> This means, without loss of generality, we can [assume](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#assuming) all **objective bids** to be denominated in the **base currency**.

<br></br>
## **Bid** Categories

[Assuming](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#assuming) that the **min** value $\underline{M}$ is also in the **base currency**, we [define](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#practical-chrony) the following 4 **bid** categories:
...
and the **[lurkers](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#why-lurkers)** never get serviced
...
- order of **objective** **bid**s is objectively [deterministic](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#deterministic)
...
First, we choose the next **bid** **category** as the **category** that brings our realized **importance** [closest](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#distance) ($\delta$) to the target **importance** as set by the seller.
...
>Either case results in a [total](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#total-ordering), deterministic and objective ordering of all the **objective** **bids** and a total subjective ordering of all the **subjective** **bid**s, both interwoven according to the chosen **importance** $\mathcal{I}$.

A discussion of the choices is found [here](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#full-subj-choice-is-better).
...
Smart contracts are autonomous, decentralized apps. The described market model is implemented as a smart contract for the following reasons: **infinite inclusivity**, zero credit risk, perfect [transparency](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#auditability-is-better), ability to use any kind of **currency**.
...
Using a **CFS**, we have increased the fungability of [any](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#fungable-nfts) **currency** $\text{ccy}$
...
By changing the parameters, minimum $\underline{M}$ and importance $\mathcal{I}$, the seller can run a [multi-dim optimisation](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#multidim-optimisation) to find the type of market that maximises it's value.
...
[**time**](https://github.com/1m1-github/FairMarket/blob/main/Notes.md#live-in-the-now)
