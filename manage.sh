...
goal clerk send --from $BIDDER --to $SELLER_FAIRMARKET_ACCOUNT --amount 239700 --note "5\n" --out $TXNS_DIR/algo_send.txn
goal app call --from $BIDDER --app-id $SELLER_FAIRMARKET_APP --foreign-asset $ASSET_ID --app-arg "str:create_bid" --app-arg $PARENT_BID --app-arg $BIDDER_FAIRMARKET_APP --app-arg $BOX_ID --box $BOX_ID --note "3\n" --fee 300000 --out $TXNS_DIR/app_call.txn
...
goal app call --from $BIDDER --app-id $SELLER_FAIRMARKET_APP --app-arg "str:add_data" --note "4\n" --out $TXNS_DIR/note_extra_1.txn
...
# cancel bid [bidder]
goal app call --from $BIDDER --app-id $SELLER_FAIRMARKET_APP --app-arg "str:cancel_bid" --app-arg $TX_ID --out $TXNS_DIR/cancel_bid.txn
